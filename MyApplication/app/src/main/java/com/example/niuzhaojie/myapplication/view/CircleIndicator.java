...
/**
 * Created by niuzhaojie on 16/10/12.
 */
...
public class CircleIndicator extends LinearLayout {
    ...
    public void setViewPager(ViewPager viewPager, int viewPagerCount) {
    ...
    private final OnPageChangeListener mInternalPageChangeListener = new OnPageChangeListener() {
        ...
        public void onPageSelected(int position) {
            position = position%mViewPagerCount;
            Log.e("tag",">>>>>>>>"+position);

            if (mViewpager.getAdapter() == null || mViewPagerCount <= 0) {
                return;
            }
    ...
    private void createIndicators() {
        removeAllViews();
        int count = mViewPagerCount;
        if (count <= 0) {
            return;
        }
        int currentItem = mViewpager.getCurrentItem();

        Log.e("tag", currentItem + "========" + count);
